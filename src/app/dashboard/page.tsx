...
const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'projects' | 'systemDesigns' | 'articles' | 'messages'>('projects');
    ...
    <div className="text-gray-900">
      ...
      {activeTab === 'projects' && (
        <div>
          ...
          <div className="mb-8">
            ...
            <ProjectForm onSuccess={() => refetchProjects()} />
          </div>
          <div>
      ...
      {activeTab === 'systemDesigns' && (
        <div>
          ...
          <div className="mb-8">
            <h3 className="text-xl font-semibold mb-4">Add New System Design</h3>
            <SystemDesignForm onSuccess={() => refetchSystemDesigns()} />
          </div>
          <div>
      ...
      {activeTab === 'articles' && (
        <div>
          ...
          <div className="mb-8">
            ...
            <ArticleForm onSuccess={() => refetchArticles()} />
          </div>
          <div>
      ...
      {activeTab === 'messages' && (
        <div>
          ...
          <div>
            {messagesData?.success && messagesData.messages?.map((message: any) => (
              <div key={message._id} className="border border-gray-300 p-4 mb-4 rounded-lg hover:shadow-lg transition">
                <h4 className="text-lg font-bold mb-2">{message.name} ({message.email})</h4>
...
function SystemDesignForm({ onSuccess }: { onSuccess: () => void }) {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [githubLink, setGithubLink] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createSystemDesign({
      title,
      description,
      githubLink,
    });
