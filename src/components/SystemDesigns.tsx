    ...
    <div className="py-5">
      ...
      <div className="flex flex-col gap-y-6 mt-3">
        {systemDesigns.length > 0 ? (
          systemDesigns.slice(0, 3).map((study: any, idx: number) => (
            <SystemDesignCard
              key={idx}
              title={study.title}
      ...
      <div className="md:text-left text-center">
        <Link href="/system-designs">
          <button className="text-white font-body mx-auto cursor-pointer bg-background px-6 py-2 mt-10 rounded-sm transition-colors duration-200 border border-white hover:border-black hover:bg-white hover:text-black">
            View More
          </button>
        </Link>
      </div>
